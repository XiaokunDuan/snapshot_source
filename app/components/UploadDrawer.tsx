            ...
            <Drawer.Portal>
                ...
                <Drawer.Content className="fixed bottom-0 left-0 right-0 z-50 mx-auto mt-24 flex max-w-md flex-col rounded-t-[36px] border border-[var(--editorial-border)] bg-[var(--editorial-paper)] outline-none shadow-[0_-24px_80px_rgba(0,0,0,0.18)]">
                    <Drawer.Title className="sr-only">
                        {locale === 'en' ? 'Choose an upload source' : '选择上传方式'}
                    </Drawer.Title>
                    <div className="flex-1 rounded-t-[36px] bg-[var(--editorial-paper)] p-5">
                        ...
                        <div className="mx-auto max-w-md px-1 pb-6">
                            <div className="mb-6">
                                ...
                                <h3 className="editorial-serif mt-4 text-3xl font-semibold tracking-[-0.04em] text-[var(--editorial-ink)]">
                                    {locale === 'en' ? 'Choose an image source' : '选择图像入口'}
                                </h3>
                                <p className="mt-3 text-sm leading-7 text-[var(--editorial-muted)]">
                                    {locale === 'en'
                                        ? 'Use the camera or gallery to bring one new concept into the learning desk. The next steps compress, analyze, and archive it automatically.'
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <h4 className="editorial-serif mt-1 text-2xl font-semibold text-[var(--editorial-ink)]">
                                                {locale === 'en' ? 'Camera' : '拍照'}
                                            </h4>
                                            <p className="mt-1 text-sm text-[var(--editorial-muted)]">
                                                {locale === 'en' ? 'Capture the current scene with the camera.' : '使用相机记录当前场景里的单词。'}
                                            </p>
                                        </div>
                                    </div>
                                </button>
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <h4 className="editorial-serif mt-1 text-2xl font-semibold text-[var(--editorial-ink)]">
                                                {locale === 'en' ? 'Gallery' : '相册'}
                                            </h4>
                                            <p className="mt-1 text-sm text-[var(--editorial-muted)]">
                                                {isNative
                                                    ? (locale === 'en' ? 'Pick one photo from the gallery and continue the analysis.' : '从手机相册选择一张图继续识别。')
                                                    : (locale === 'en' ? 'Choose an image file. Common formats are supported.' : '选择图片文件，常见格式都支持。')}
                                            </p>
                                        </div>
                                    </div>
                                </button>
                            ...
                            <div className="mt-5 rounded-[24px] border border-[var(--editorial-border)] bg-[rgba(255,251,244,0.78)] px-4 py-3">
                                <p className="text-xs leading-6 text-[var(--editorial-muted)]">
                                    {locale === 'en'
                                        ? 'On the web, Snapshot prioritizes image types the current browser can decode and compresses them before analysis.'
                        ...
                        <div className="mt-4 border-t border-[var(--editorial-border)] pt-4">
                            <Drawer.Close asChild>
                                <button className="w-full py-3 text-center font-medium text-[var(--editorial-muted)] transition-colors hover:text-[var(--editorial-ink)]">
                                    {locale === 'en' ? 'Cancel' : '取消'}
                                </button>
                            </Drawer.Close>
                        </div>
